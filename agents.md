...
- **자율 실행 및 자체 승인 (Autonomous Execution)**: 사용자의 수동 승인(Accept) 절차를 최소화하고 작업 속도를 극대화하기 위해, 코드 수정, 파일 생성, 검증, 디버깅 등의 작업은 중간 질문이나 불필요한 승인 대기 없이 에이전트가 자체 판단하여 끝까지 완결 후 결과만 명확히 보고합니다.
